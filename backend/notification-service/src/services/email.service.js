...
logger.info(`- EMAIL_PASS: ${process.env.EMAIL_PASS ? 'set (length: ' + process.env.EMAIL_PASS.length + ')' : 'not set'}`);
...
async function sendEmail(to, subject, htmlContent) {
  try {
    ...
    if (process.env.NODE_ENV === 'development' && process.env.USE_ETHEREAL === 'true') {
      ...
      logger.info(`Email sent successfully with Ethereal! Preview URL: ${nodemailer.getTestMessageUrl(info)}`);
      return info;
    } else {
