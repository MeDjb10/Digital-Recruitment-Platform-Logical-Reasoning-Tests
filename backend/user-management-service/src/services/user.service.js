...
exports.updateTestAuthorizationStatus = async (
  ...
  if (examDate) {
    additionalData.examDate = new Date(examDate);
  }

  // Update the user with status change only
  const updatedUser = await User.findByIdAndUpdate(
    userId,
    { $set: updateData },
    { new: true }
  ...
  // If approved, publish an event for test assignment
  if (status === "approved") {
    ...
    logger.info(`Published candidate approved event for user ${userId}`);
  }

  // Send status change email with test assignment info if approved
  try {
    await publishMessage(
      "notification_events", // Use the notification exchange
    ...
    logger.info(
      `Published email notification request for user ${userId} with status ${status}`
    );
  } catch (error) {
    logger.error(`Failed to publish email notification for user ${userId}`, {
      error: error.message,
    });
  }
...
exports.getUserTestAssignment = async (userId) => {
  ...
  // Check if user has a test assignment
  if (!user.testAssignment || user.testAssignment.assignedTest === "none") {
    throw new ErrorResponse("No test assignment found for this user", 404);
  }

  return user.testAssignment;
};
