...
export class AuthService {
  ...
  login(email: string, password: string, rememberMe: boolean = false): Observable<any> {
    return this.http.post<AuthResponse>(`${this.apiUrl}/login`, { email, password })
      .pipe(
        tap(response => {
          // Store tokens
          this.storeTokens(response.accessToken, response.refreshToken, rememberMe);
  ...
  verifyResetOTP(email: string, otp: string, resetToken: string): Observable<PasswordResetResponse> {
    return this.http.post<PasswordResetResponse>(`${this.apiUrl}/verify-reset-otp`, { 
      email, 
      otp, 
      resetToken 
    }).pipe(
  ...
  requestLoginOTP(email: string): Observable<any> {
    return this.http.post(`${this.apiUrl}/request-login-otp`, { email })
      .pipe(
        catchError(error => {
  ...
  verifyLoginOTP(email: string, otp: string): Observable<AuthResponse> {
    return this.http.post<AuthResponse>(`${this.apiUrl}/verify-login-otp`, { email, otp })
      .pipe(
        tap(response => {
          // Store tokens
